...
for record in data:
    ...
    if prev_tree is not None:
        record = [ast.literal_eval(print_tree(prev_tree)), record]
        print(f"After adding {record}")
    r_tree = Node(record, None, 0)
    print(print_tree(r_tree))
    while True:
        ops = []
        recurse(r_tree, ops)
        if len(ops) == 0:
